...
describe('judicial process list', () => {
  it('agrupa pela relação nominal e abre todos os polos em um dialog', async () => {
    ...
    expect(screen.getByRole('columnheader', { name: 'Polo passivo' })).toBeInTheDocument()
    ...
    await userEvent.click(screen.getByText('Governador Valadares'))
    expect(screen.getByRole('dialog')).toBeInTheDocument()
    expect(screen.getByText(/310\.000,00/)).toBeInTheDocument()
    expect(screen.getAllByText('Polo ativo')).toHaveLength(2)
    expect(screen.getAllByText('Polo passivo')).toHaveLength(2)
    ...
    expect(screen.getAllByText('OUTRA PARTE')).toHaveLength(2)
