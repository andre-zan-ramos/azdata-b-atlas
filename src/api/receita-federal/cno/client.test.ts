...
describe('contrato CNO', () => {
  it('usa allowlist por endpoint sem transformar identificadores', () => {
    expect(serializeParams('obras', { cno: '00.01', ni_responsavel: ' 000-X ', uf: '', q: 'não', release: 'x', return_to: '/', page: 1, include_total: false }).toString()).toBe('cno=00.01&ni_responsavel=+000-X+&page=1&include_total=false')
  ...
  it('usa as cinco rotas, propaga signal e nunca envia query no detalhe', async () => {
    ...
    await api.obras({ cno: '0001' }, signal); await api.areas({ cno: '0001' }, signal); await api.cnaes({ cnae: '001' }, signal); await api.vinculos({ ni_responsavel: '00-X' }, signal)
    expect(await api.obra('0123', signal)).toEqual(data)
    expect(adapter.mock.calls.map(([config]) => config.url)).toEqual(['obras/', 'areas/', 'cnaes/', 'vinculos/', 'obras/0123/'].map(path => `api/v1/receita-federal/cno/${path}`))
    expect(adapter.mock.calls.every(([config]) => config.signal === signal && config.baseURL === 'https://configurada.example/')).toBe(true)
    expect(adapter.mock.calls[4][0].params).toBeUndefined()
